...
running inside the Puma process and Solid Cache in the same database, Hotwire, Ed25519
signatures over RFC 8785 canonical JSON, and Docker Compose with exactly two services.
One app, one database, no external service required.
...
| [galedra.org/assistants/new](https://galedra.org/assistants/new) | Connect an assistant: ChatGPT, Claude, or any MCP client |
| [galedra.org/docs](https://galedra.org/docs) | This README, with pointers to the spec, the skill, and the log |
...
| [galedra.org/bug_reports/new](https://galedra.org/bug_reports/new) | Report a bug. People and assistants file them the same way |

Three more that are worth knowing and are not in that menu:

| Page | What is there |
|------|---------------|
| [galedra.org/api/v1/openapi.json](https://galedra.org/api/v1/openapi.json) | The OpenAPI description itself, for GPT Actions and plain HTTP clients |
...
| 14 | `14-federation-goal.md` | What federation has to preserve, and what the data structures must carry for it |
...
`examples/watchers/`, inside that same directory, is an internal stress test over a
textual source.
...
This is the P0 bar, and `v0.1.0` meets every item: a user can import a source and mark an exact
...
`bin/demo --reset` checks it, printing PASS for every golden row and the replay check.
Each of those steps is a signed entry in the same log as everything else.
...
One third-party bundle travels with the source: `vendor/assets/swagger-ui/` holds Swagger UI,
Apache-2.0, which renders the API reference without reaching a CDN. Its own licence and notice
sit beside it, and [NOTICE](NOTICE) records it.
