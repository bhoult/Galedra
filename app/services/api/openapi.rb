    ...
    def read_paths
      {
        ...
        "/api/v1/claims/{id}/score" => get_op("getClaimScore", "The probability with its model and snapshot; never a percentage true. `unchanged_since` appears when nothing has borne on the claim since an earlier seq.", params: [ path_id, query("model"), snapshot_seq_query ]),
        "/api/v1/claims/{id}/trace" => get_op("getClaimTrace", "The whole scoring trace with its hash: every step that produced the number, reproducible from the same seq and model. The trace returned is the one computed at the last seq that bore on this claim, which may be earlier than the seq asked for; `unchanged_since` then names it, and nothing is restamped with a seq at which it was not computed.", params: [ path_id, query("model"), snapshot_seq_query ]),
