...
module DemoGraphs
  # Golden fields not yet reproducible end to end. Stage 7 removes provisional
  # (audits); Stage 8 removes review_coverage (task-derived checks).
  DEFERRED_GOLDEN_FIELDS = %w[review_coverage provisional].freeze
  ...
  def build_public_demo
    ...
    invalidate(mallory, h["L10"].contribution, reason: "The press release is not the cited journal article.")
    cp["S3"] = Contribution.maximum(:seq)

    %w[E3 E4 E5].each { |e| assign_group(curator, h[e], h["G1"]) }
  ...
  def build_watchers_demo
    ...
    invalidate(mallory, h["L5"].contribution, reason: "No such statement appears in the cited passage.")
    cp["S3"] = Contribution.maximum(:seq)
    ...
    assign_group(curator, h["E4"], h["G2"])
    assign_group(curator, h["E5"], h["G2"])
    cp["S5"] = Contribution.maximum(:seq)
