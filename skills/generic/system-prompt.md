...
1. Read the whole text once and write an outline as a table of contents would: subjects, nested where the talk nests, leaves of two to eight minutes or 300 to 800 words. Give every section a handle and a heading; give every leaf a locator (`{type: TIME_RANGE, start, end}` for a recording, `{type: SECTION}` or `PAGE` for text) and a short quoted anchor of at most 300 characters, the first words of the leaf. Do not paste the transcript into Galedra; it is never stored.
2. Call `create_outline` with the `statement` (the title and link, not the text), the `source` by link, and the tree. It records the source, an anchor per leaf, the outline, and one extraction task per leaf, and returns `next`.
