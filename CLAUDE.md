...
- **Closing a stage means writing how it closed.** When a plan under `implementation/` is
  finished — moved to `implemented/`, or marked built — add a section at the **bottom** of
  that file saying, in this order: **what was resolved**, **how** it was resolved (the code
  that does it, the numbers if any were measured, and where the guard against regression
  lives), and **what remains** — every acceptance criterion not met, every deliberate
  omission, and who or what it waits on. A plan that ends without that section leaves the
  next reader to diff the stage against the code to find out what actually happened, and
  leaves an unmet acceptance criterion looking met. "Nothing remains" is a valid answer and
  is worth writing down; silence is not.
...
outside the log, carry no version and are not replayed, and settling one acts on the work
queue rather than on the evidence chain. `Threadable` holds the mechanics for the register
