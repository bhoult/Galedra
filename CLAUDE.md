...
`bench:boot`. `bin/grafana` (and `bin/grafana stop`) shows the request metrics on a dashboard at 127.0.0.1:3001 — a standalone container on the Compose network, not a third service, reading through a role that can see only the two metrics tables. **Write up a run that changed what we believe as a new `docs/profiler/YYYY-MM-DD-<subject>.md`**
